...
def scheme_eval(expr, env, _=None):  # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 4
        "*** YOUR CODE HERE ***"
        the_operator = scheme_eval(first, env)
...
class Frame(object):
    ...
    def lookup(self, symbol):
        ...
        frame = self
        while frame:
            found_value = frame.bindings.get(symbol, None)
    ...
    def apply(self, args, env):
        ...
        def helper(linked_list):
            if linked_list == nil:
                ...
                return [linked_list.first] + helper(linked_list.rest)

        python_args.extend(helper(args))
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 5
        "*** YOUR CODE HERE ***"
        exprs = scheme_eval(expressions.rest.first, env)
        env.define(target, exprs)
        return target
        # END PROBLEM 5
        ...
        lambs = do_lambda_form(Pair(formals, body), env)
        env.define(name, lambs)
        return name
        # END PROBLEM 9
    else:
